...
#ifdef DEBUG
  /*
   * The debugger loads both the FSBL and the application directly into AXI
   * SRAM.  Skip probing the external NOR in this configuration so a stale
   * flash protocol state left by a previous debug session cannot delay the
